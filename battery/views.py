...
class BatteryBatchViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [AdminSales]  # Updated to allow Sales users
    ...
    def generate_batteries(self, request, pk=None):
        ...
        existing = batch.batteries.count()
        if existing >= batch.quantity: 
            return Response({"detail": "All batteries already generated.", "generated": existing})
        for i in range(existing + 1, batch.quantity + 1):
            Battery.objects.create(
                battery_id=f"{batch.batch_number}-BAT-{i:03d}",
                serial_number=f"{batch.batch_number}-SER-{i:04d}",
                batch=batch,
                model=batch.battery_model,
                battery_type=batch.battery_type,
                g_code=batch.g_code,
                manufacturing_date=batch.manufacturing_date,
                status=Battery.Status.IN_STOCK,
                warranty_start=batch.manufacturing_date,
                warranty_end=batch.manufacturing_date.replace(year=batch.manufacturing_date.year + max(1, batch.warranty_period_months // 12))
            )
        batch.status = BatteryBatch.Status.COMPLETED
        batch.save(update_fields=["status"])
        return Response(BatteryBatchSerializer(batch).data)
...
class BatteryViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self): 
        return BatteryDetailSerializer if self.action == "retrieve" else BatterySerializer
    ...
    def status(self, request, pk=None):
        ...
        if new not in valid: 
            return Response({"detail": "Invalid battery status."}, status=400)
        old = b.status
        b.status = new
    ...
    def assign_vendor(self, request, pk=None):
        ...
        b = self.get_object()
        try: 
            vendor = Vendor.objects.get(pk=request.data.get("vendor"))
        except Vendor.DoesNotExist: 
            return Response({"detail": "Vendor not found"}, status=404)
        old = b.status
        b.vendor = vendor
        b.status = Battery.Status.ASSIGNED_VENDOR
        b.sales_person = request.user
        b.save()
    ...
    def assign_customer(self, request, pk=None):
        ...
        old = b.status
        b.customer = customer
        b.status = Battery.Status.ASSIGNED_CUSTOMER
        b.save()
    ...
    def assign_scooter(self, request, pk=None):
        ...
        b = self.get_object()
        try: 
            scooter = Scooter.objects.get(pk=request.data.get("scooter"))
        except Scooter.DoesNotExist: 
            return Response({"detail": "Scooter not found"}, status=404)
        old = b.status
        b.scooter = scooter
        b.customer = scooter.customer
        b.vendor = scooter.vendor
        b.status = Battery.Status.INSTALLED
        b.save()
        return Response(BatteryDetailSerializer(b).data)
...
def public_battery(request, serial):
    try: 
        b = Battery.objects.select_related("batch", "vendor", "customer", "scooter").get(serial_number=serial)
    except Battery.DoesNotExist: 
        return Response({"detail": "Battery not found"}, status=404)
    return Response({
        ...
        "warranty_start": b.warranty_start,
        "warranty_end": b.warranty_end,
        "status": b.status,
        "vendor": b.vendor.business_name if b.vendor else None,
        "customer": b.customer.name if b.customer else None,
        "scooter": b.scooter.scooter_id if b.scooter else None
    })